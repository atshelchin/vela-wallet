// Ported from src/services/abi.ts @ c13e89d4 (spec 025).
/**
...
function h(v: bigint | number, bytes = 32): string {
	void bytes; // the port's signature; every word is 32 bytes
...
export function decAggregate3(hex: string): McResult[] {
	...
	for (let i = 0; i < len; i++) {
		...
		const data = bLen > 0 && dataEnd <= d.length ? '0x' + d.slice(bPos + 64, dataEnd) : '0x';
...
export function encQuoteV3(
	...
	return '0x' + SEL.quoteV3 + addr(tokenIn) + addr(tokenOut) + h(amountIn) + h(fee) + h(0n); // sqrtPriceLimitX96 = 0
}
