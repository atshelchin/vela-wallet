// Ported from src/services/approval-guard.ts @ f9bcb278 — RN seams rewritten to the web modules; logic verbatim.
/* eslint-disable @typescript-eslint/no-explicit-any -- verbatim port: these walk dynamic wire
   shapes (decoded params, eth_simulateV1 results, selector DB rows) that Expo typed `any`;
   narrowing them is a rewrite, not a port (spec 026 results.md, Phase 2). */
import { abiEncodeUint256, functionSelector, stripHexPrefix, toHex } from '$lib/core/kernels';
/**
...
export interface DetectedApproval {
	...
	locus: { type: 'calldata-word'; wordIndex: number } | { type: 'typed-path'; path: string };
}
...
function detectCalldataApproval(
	to: string | undefined,
	...
	if (selector === SELECTORS.approve) {
		...
		return {
			kind: 'erc20-approve',
			tokenAddress: to?.toLowerCase(),
			spender,
			amountRaw,
			amountBits: 256,
			isUnbounded: isUnboundedAmount(amountRaw, 256),
			isBooleanGrant: false,
			isReducing: amountRaw === 0n,
			editable: true,
	...
	if (selector === SELECTORS.permit2Approve) {
		...
		return {
			...
			amountRaw,
			amountBits: 160,
			isUnbounded: isUnboundedAmount(amountRaw, 160),
			isBooleanGrant: false,
			isReducing: amountRaw === 0n,
			editable: true,
			deadline: bigFromWord(word(3)),
			locus: { type: 'calldata-word', wordIndex: 2 }
		};
	}
...
function detectTypedDataApproval(td: any): DetectedApproval | null {
	...
	if (pt === 'Permit' && msg.allowed !== undefined) {
		const allowed =
			msg.allowed === true || msg.allowed === 'true' || msg.allowed === 1 || msg.allowed === '1';
		return {
			kind: 'dai-permit',
			tokenAddress: lc(domain.verifyingContract),
			spender: lc(msg.spender),
			isUnbounded: allowed,
			isBooleanGrant: true,
			isReducing: !allowed,
			editable: false,
			blockReason: allowed
				...
				: undefined,
			deadline: toBig(msg.expiry),
			locus: { type: 'typed-path', path: 'allowed' }
		};
	}
	...
	if (pt === 'Permit' && msg.value !== undefined) {
		...
		return {
			...
			deadline: toBig(msg.deadline),
			locus: { type: 'typed-path', path: 'value' }
		};
	}
	...
	if (pt === 'PermitSingle' && msg.details) {
		...
		return {
			kind: 'permit2-single',
			tokenAddress: lc(msg.details.token),
			spender: lc(msg.spender),
			amountRaw,
			amountBits: 160,
			isUnbounded: isUnboundedAmount(amountRaw, 160),
			...
			editable: false,
			blockReason: PERMIT_SIG_BLOCK,
			deadline: toBig(msg.details.expiration),
			locus: { type: 'typed-path', path: 'details.amount' }
		};
	}
...
export function rewriteApprovalParams(
	...
	if (method.includes('signTypedData')) {
		const idx =
			typeof params[1] === 'string' || (params[1] && typeof params[1] === 'object') ? 1 : 0;
...
function chosenAmount(detected: DetectedApproval, choice: ApprovalChoice): bigint {
	...
	if (choice.type === 'grant') {
		if (!detected.isBooleanGrant)
			throw new Error('grant choice is only valid for boolean approvals');
...
export class UnlimitedApprovalError extends Error {
	constructor(detail: string) {
		super(
			`Blocked: this would grant an unlimited approval (${detail}). Set a finite amount and try again.`
		);
