// Ported from src/services/bundler-service.ts @ f9bcb278 — RN seams rewritten to the web modules; logic verbatim.
/**
 * Gas account service.
 *
...
export async function checkBundlerFunding(
	...
	console.log(
		`[BundlerFunding] account info:`,
		info
			? `deposit=${info.depositAddress} balance=${info.spendableBalance} status=${info.status}`
			: 'unreachable'
	);
	...
	console.log(
		`[BundlerFunding] threshold=${threshold} spendable=${info.spendableBalance} sufficient=${info.spendableBalance >= threshold} (gasCost=${estimatedGasCostWei ?? 'default'})`
	);
...
export type SilentSponsorship =
	/** Gas account is usable — proceed without unknown funding UI. `sponsored` is
...
export async function attemptSilentSponsorship(
	...
	if (
		...
		(globalThis as { __VELA_PARALLEL__?: boolean }).__VELA_PARALLEL__
	) {
...
export async function probeGasSponsorship(funding: FundingNeeded): Promise<SponsorProbe> {
	...
	try {
		...
		if (!res.ok) {
			let reason =
				res.status === 503 || res.status === 429 ? 'service_unavailable' : 'request_failed';
			try {
		...
		if (body.reason === 'pending_unknown' || body.reason === 'already_in_progress')
			return { outcome: 'confirming' };
		if (body.reason === 'already_funded') return { outcome: 'eligible' };
		return { outcome: 'denied', reason: body.reason };
	} catch {
		// The probe is advisory — on unknown transport failure defer the decision to
	...
	try {
		...
		if (isTempoChain(chainId) && data.activeDepositAddress) {
			try {
				...
				const balRes = await poolRpcCall(
					'eth_call',
		...
		const settlementRecipient =
			typeof data.settlementRecipient === 'string' &&
			/^0x[0-9a-fA-F]{40}$/.test(data.settlementRecipient)
...
function parseBigIntHex(value: unknown): bigint {
	if (!value) return 0n;
 ...
 * balances to 18 decimals — comparing across units would make unknown dust
...
const inBandQuoteRequests = new Map<
	string,
...
async function requestInBandGasQuotes(
	...
	const request = fetchInBandGasQuotesDetailed(chainId, safeAddress)
		...
		.finally(() => {
			inBandQuoteRequests.delete(key);
		});
 ...
 * native; unknown contract means that exact ERC-20 asset. */
...
async function fetchInBandGasQuotesDetailed(
	...
	try {
		const resp = await poolBundlerCall('vela_getInBandGasQuote', [{ safeAddress }], chainId);
		if (resp.error || !resp.result) {
			const notEnabled =
				/not enabled/i.test(resp.error?.message ?? '') || resp.error?.code === -32601; // method not found — pre-in-band bundler
			console.log(
				`[InBand] quote unavailable on chain=${chainId}: ${resp.error?.message ?? 'empty result'}`
			);
		...
		const quotes = resp.result.flatMap((r: Record<string, unknown>): InBandGasQuote[] => {
			...
			const recipient =
				typeof r.recipient === 'string' && /^0x[0-9a-fA-F]{40}$/.test(r.recipient)
					? r.recipient
					: null;
			const asset = r.asset === 'native' || r.asset === 'erc20' ? r.asset : null;
			const feeToken =
				typeof r.feeToken === 'string' && /^0x[0-9a-fA-F]{40}$/.test(r.feeToken)
					? r.feeToken
					: null;
			const balance = parseBigIntHex(r.balance);
			const decimals =
				typeof r.decimals === 'number' && Number.isSafeInteger(r.decimals) && r.decimals >= 0
					? r.decimals
					: null;
		...
		const usableQuotes =
			nativeQuote?.usdPrice === null ? quotes.filter((quote) => quote.asset === 'native') : quotes;
