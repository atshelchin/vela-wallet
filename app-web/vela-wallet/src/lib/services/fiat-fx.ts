/**
 * Fiat FX rates fetched directly on the client from the user-configurable
 * endpoint (`fiatRatesURL`, default Vela's Frankfurter v2 — FOSS, ~160
 * currencies incl. VND). Ported from src/services/fiat-fx.ts @ c13e89d4
 * (AsyncStorage → the KV; info logs dropped, warnings kept).
 *
 * The endpoint must return USD-based rates in one of two shapes
 * (`normalizeRates` accepts both, so providers are swappable):
 *   Frankfurter v2 (array):  [{ base:'USD', quote:'EUR', rate:0.92 }, …]
 *   open.er-api / v1 (object): { rates: { EUR: 0.92, … } }
 *
 * Cached in-memory (6h) + persisted for offline/first-paint; the cache is
 * keyed by endpoint URL so changing the endpoint refetches.
 */
import { getFiatRatesURL } from './endpoints';
import { fetchWithTimeout, NET_TIMEOUTS } from './net';
import { getItem, setItem } from './storage';

/** Normalize a provider response (array or `{rates}`) to `{ USD:1, X:rate }`, or null. */
export function normalizeRates(data: unknown): Record<string, number> | null {
	...
	} else if (data && typeof (data as { rates?: unknown }).rates === 'object') {
...
// ECB / Frankfurter rates update once per business day — a long cache is plenty.
...
/** USD-based rate map for every currency the endpoint returns (`{ USD: 1, EUR: 0.92, … }`). */
		...
		try {
			...
			if (res.ok) {
				...
				if (rates) {
					...
					setItem(CACHE_KEY, JSON.stringify(_cache)).catch(() => {});
					return rates;
				}
			}
...
/** Currency codes the configured endpoint can price. */
