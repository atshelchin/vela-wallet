// Ported from src/services/price-service.ts @ c13e89d4 (spec 025).
/**
	...
	try {
		...
		for (let i = 0; i < symbols.length; i++) {
			...
			if (r?.success && r.data.length >= 66) {
				...
				console.warn(
					`[Chainlink] ${symbols[i]} feed failed: success=${r?.success} dataLen=${r?.data?.length ?? 0}`
				);
			}
		}

		console.log(
			`[Chainlink] prices: ${Object.entries(prices)
