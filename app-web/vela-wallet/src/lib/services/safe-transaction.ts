// Ported from src/services/safe-transaction.ts @ f9bcb278 — RN seams rewritten to the web
// modules; every byte of logic verbatim. This file builds, signs and submits the UserOp:
// a rewrite here is a fund-safety event, so the port is literal and the vector suite
// (safe-transaction.test.ts) came with it.
/* eslint-disable @typescript-eslint/no-explicit-any, @typescript-eslint/no-unused-vars, preserve-caught-error
   -- verbatim port: the JSON-RPC answers read here (blocks, gas quotes, receipts, bundler
   errors) are dynamic wire shapes Expo typed `any`; two helpers are kept for parity with
   the Expo module's exports; the four re-thrown estimation errors deliberately replace the
   transport's words with the person's. Recorded in spec 026 results.md, Phase 2. */
/**
...
export function signerAddressFor(signer: WalletSigner, credentialId: string | null): string {
	...
	const index = signer.keys.findIndex(
		(key) => normalize(key.credentialId) === normalize(credentialId)
	);
	if (index < 0) {
...
export async function sendNative(
	...
	if (isTempoChain(chainId)) {
		...
		return sendUserOpTempo(
			from,
			[{ to, value: valueWei, data: new Uint8Array(0) }],
			gasFeeToken ?? TEMPO_DEFAULT_FEE_TOKEN,
			chainId,
			publicKeyHex,
			signFn,
			quotedFee
...
export async function sendERC20(
	...
	if (isTempoChain(chainId)) {
		return sendUserOpTempo(
			from,
			[{ to: tokenAddress, value: '0', data: transferData }],
			gasFeeToken ?? TEMPO_DEFAULT_FEE_TOKEN,
			chainId,
			publicKeyHex,
			signFn,
...
export function sameAssetFeeLimit(
	...
	if (fee.feeAsset?.kind === 'erc20') {
		if (!transferAsset || fee.feeAsset.token.toLowerCase() !== transferAsset.toLowerCase())
			return null;
...
export function calculateInBandFeeAmount(
	...
	const admissionFloor = nativeAsset.decimals >= 5 ? 10n ** BigInt(nativeAsset.decimals - 5) : 1n;
	...
	const nativeMinimum = nativeUsdPrice
		...
		: nativeAsset.decimals >= 3
			? 10n ** BigInt(nativeAsset.decimals - 3)
			: 1n;
...
async function estimateTempoFee(
	...
	if (hasContractCall && deployed) {
		try {
			...
			const innerCalls: MultiSendCall[] = actualCalls.map((call) => ({
				...
				data:
					call.data && call.data !== '0x' ? fromHex(stripHexPrefix(call.data)) : new Uint8Array(0)
			}));
			...
			expectedGas = bigintMax(
				expectedGas,
				est.verificationGasLimit + est.callGasLimit + est.preVerificationGas
			);
		} catch (err) {
	...
	return {
		...
		feeAsset: {
			...
			symbol:
				feeToken.toLowerCase() === TEMPO_DEFAULT_FEE_TOKEN.toLowerCase() ? 'pathUSD' : undefined
		},
		feeRecipient:
			feeRecipient && /^0x[0-9a-fA-F]{40}$/.test(feeRecipient) ? feeRecipient : undefined
	};
}
...
export async function estimateTransactionFee(
	...
	// The bundler is the single source of truth for the price. Its quoted price is
	// shown on the confirmation surface; only an unavailable quote falls back locally.
	...
	if (!estimateAccount) {
		throw new Error(
			'Could not load the passkey public key required to build the UserOperation initCode'
		);
	}
	...
	if (!inBandQuote || !nativeInBandQuote) {
		if (gasFeeToken) {
			throw new Error(
				'The gas relayer cannot quote the selected fee token right now. Please pick a different gas asset.'
			);
		}
	...
	try {
		...
		console.log(
			`[FeeEstimate] Bundler estimation unavailable, using defaults:`,
	...
	const feeAmount = calculateInBandFeeAmount(
		totalGas,
		inBandGasBasis,
		inBandQuote,
		nativeInBandQuote
	...
	const common = {
		...
		feeRecipient: inBandQuote.recipient
	};
...
async function sendUserOpTempo(
	...
	const calculatedReimbursement = tempoReimbursement(
		realisticGas,
		gasPrices.gasPrice,
		TEMPO_FEE_TOKEN_DECIMALS
	);
	...
	const reimbursement =
		quotedFee && quotedFee.amount > 0n && /^0x[0-9a-fA-F]{40}$/.test(quotedFee.recipient)
			? quotedFee.amount
			: calculatedReimbursement;
	userOp.callData = buildBatch(reimbursement);
	console.log(
		`[Tempo] feeToken=${feeToken} reimbursement=${reimbursement} calculated=${calculatedReimbursement} displayed=${quotedFee ? 'yes' : 'no'} realisticGas=${realisticGas} staticGas=${staticGas} est=${estActualGas ?? 'n/a'} collector=${feeCollector}`
	);
...
async function sendUserOpInBand(
	...
	const buildBatch = (amount: bigint, recipient: string): Uint8Array =>
		buildMultiSendExecuteCallData([
			...innerCalls,
			buildInBandFeeLeg(gasFeeToken, recipient, amount)
		]);
	...
	if (quotedFee && quotedFee.amount > 0n && /^0x[0-9a-fA-F]{40}$/.test(quotedFee.recipient)) {
		...
		console.log(
			`[InBand] signing DISPLAYED quote: feeToken=${gasFeeToken ?? 'native'} amount=${feeAmount} recipient=${feeRecipient}`
		);
	} else {
		const quotes = await fetchInBandGasQuotes(chainId, safeAddress);
		...
		console.log(
			`[InBand] feeToken=${gasFeeToken ?? 'native'} amount=${feeAmount} recipient=${feeRecipient} gas=${totalGas} gasPrice=${gasPrice}`
		);
	}
...
export function buildExecuteCallData(to: string, value: string, data: Uint8Array): Uint8Array {
 ...
 */
function buildNativeCallData(innerCalls: MultiSendCall[], alwaysMultiSend: boolean): Uint8Array {
...
function assembleInitCode(setupData: Uint8Array, saltNonce: Uint8Array): Uint8Array {
	...
	const selector = functionSelector('createProxyWithNonce(address,bytes,uint256)');
...
export function buildUserOpSignature(
	...
	const core = buildContractSignatureCore(
		authenticatorData,
		clientDataFields,
		sigR,
		sigS,
...
function buildContractSignatureCore(
	...
	const dynamicData = abiEncodeWebAuthnSig(authenticatorData, clientDataFields, sigR, sigS);
	const dataLength = abiEncodeUint256(BigInt(dynamicData.length));

	return concatBytes(rField, sField, vField, dataLength, dynamicData);
}
...
function buildDummySignature(): Uint8Array {
	...
	const fakeClientFields = '"origin":"https://getvela.app","crossOrigin":false';
...
export interface ChainGasPrice {
	 ...
	 * derived from eth_gasPrice, defaulted, or skipped (Tempo). It is retained as
	 * diagnostic context for the gas-price trace.
	 */
	tipMeasured: boolean;
}
...
async function getGasPrices(chainId: number): Promise<ChainGasPrice> {
	...
	if (cached && Date.now() - cached.at < GAS_PRICE_CACHE_TTL) {
		return {
			...
			tipMeasured: cached.tipMeasured
		};
	}
	...
	try {
		...
		const block = blockRes?.result as { baseFeePerGas?: string } | null | undefined;
		const baseFee = block?.baseFeePerGas ? parseHexUInt64(block.baseFeePerGas) : 0n;
 ...
 * bundler) so the caller can fall back to a local estimate. A usable relay
 * quote is always returned for the confirmation UI to display and review.
 */
export async function getBundlerGasQuote(
	...
	if (maxFeePerGas <= 0n) {
		console.warn(
			`[Gas] Bundler quoted ${tier} maxFeePerGas = 0 on chain ${chainId} — ignoring quote, using local estimate.`
		);
		return null;
	}
...
async function submitUserOp(
	...
	if (!initCodePresent) {
		...
		if (!deployed) {
			console.error(
				`[UserOp] ABORT pre-submit: sender ${userOp.sender} has NO on-chain code but the op carries NO initCode → would revert AA20. Refusing to submit.`
			);
			throw new Error(
