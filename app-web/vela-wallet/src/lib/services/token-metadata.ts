// Ported from src/services/token-metadata.ts @ c13e89d4 (spec 025).
/**
...
import { MULTICALL3, SEL, encAggregate3, decAggregate3, decString, decU8, type Call3 } from './abi';
...
export async function resolveTokenMetadata(
	...
	let toFetch = misses;
	try {
		const pairs = await Promise.all(misses.map((a) => getItem(storeKey(chainId, a))));
		const next: string[] = [];
		pairs.forEach((val, i) => {
			const a = misses[i];
	...
	for (let i = 0; i < toFetch.length; i += BATCH) {
		...
		try {
			const res = await poolRpcCall(
				'eth_call',
				[{ to: MULTICALL3, data: encAggregate3(calls) }, 'latest'],
				chainId
			);
	...
	if (toPersist.length) {
		await Promise.all(toPersist.map(([k, v]) => setItem(k, v))).catch(() => {});
	}
	return out;
}
