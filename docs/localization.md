...
> **The corpus moved.** Since spec `004-rust-i18n` the translation JSON lives in
> the Rust crate, not under `src/`. It is the single source of truth for every
> platform (app-web, app-ios, app-android, app-desktop), and everything below it
> in the tree is **generated** — see [Maintenance](#maintenance).

```
rust/crates/vela-core/i18n/
  locales/          # ← THE SOURCE OF TRUTH. Hand-edited. 15 languages.
    en.json         #   CORE namespaces: common / language / settings / time / activity
...
src/i18n/
  ...
  resources.ts      # GENERATED by `npm run gen:i18n` — do not hand-edit
  i18next.d.ts      # makes t() keys type-checked (derives from the English file)

public/i18n/        # GENERATED — per-language bundles the web build fetches on demand
```

- **15 languages**: `en`, `zh` (Simplified), `zh-TW`, `zh-HK`, `ja`, `ko`, `vi`,
  `id`, `tr`, `es-MX`, `pt-BR`, `fr`, `de`, `it`, `ru`.
- **15 area namespaces** + core (`common`/`language`/`settings`/`time`/`activity`):
...
- **1,129 keys per language** (kept at 100% parity — see [Maintenance](#maintenance) for the exact, always-current count).
...
```ts
// resources.ts — GENERATED by `npm run gen:i18n`; edit the corpus, not this file.
...
// …
export const en = { ...enCore, ...enHome, /* …all 15 files */ };   // also the TS key source
...
1. Add the key to **`rust/crates/vela-core/i18n/locales/en/<area>.json`** (this is
   the type source — `tsc` now requires it everywhere).
2. Add the same key to the other 14 `<lang>/<area>.json` files (translated).
3. Run **`npm run gen:i18n`** and commit its output with the corpus edit.
4. Use `t('<area>.<key>')`.
5. Run the [parity check](#maintenance) + `tsc`.
...
1. Copy the English files as the structural base (guarantees key parity):
   ```bash
   L=nl
   C=rust/crates/vela-core/i18n/locales
   cp $C/en.json $C/$L.json
   mkdir $C/$L
   for f in $C/en/*.json; do cp "$f" "$C/$L/$(basename "$f")"; done
   ```
...
3. Add it to `LOCALES` in `scripts/gen-i18n.mjs`, and add an `i18n-<lang>` feature
   to `rust/crates/vela-core/Cargo.toml` if the language should be embeddable.
4. Translate the values (keep keys + `{{placeholders}}` identical). Check its
   CLDR plural categories: a language with `few`/`many` needs those suffixed keys
   or it silently falls back to `other` — that was FR-017's bug.
5. `npm run gen:i18n` to regenerate `resources.ts`, `public/i18n/`, the Rust path
   table and the compiled-in catalogs. **Commit the corpus edit and the
   regenerated artefacts together** — CI fails the pair if they drift apart.
6. Parity check + `tsc`. The picker (`SettingsScreen`) lists languages from
...
**`npm run gen:i18n`** — regenerates every downstream artefact from the corpus.
Run it after ANY corpus edit and commit the result in the same commit; CI fails
the pair if they drift.

**`npm run lint:i18n`** — the corpus defect register (A1–A6: keys missing from a
locale, missing/dead CLDR plural categories, inconsistent plural shape, `{{count}}`
without plural siblings, load-bearing whitespace). It fails only on *new*
occurrences against `scripts/i18n-corpus-baseline.json`, so it does not block on
the debt it inventories.

**`npm run verify:i18n`** — 17,115 exhaustive + 50,000 fuzzed comparisons between
the Rust engine and the `i18next` the app still ships. A divergence is a release
blocker, not a flake.

**Placeholder parity** — not covered by the above: every language must carry the
same `{{placeholders}}` as English, and no extra keys.

```bash
node -e '
const fs=require("fs");
const C="rust/crates/vela-core/i18n/locales";
const NS=["home","send","receive","assets","addToken","tokenDetail","history","onboarding","connect","about","clearSigning","componentsTx","componentsUi","settingsModals","contacts"];
const LANGS=["en","zh","zh-TW","zh-HK","ja","ko","vi","id","tr","es-MX","pt-BR","fr","de","it","ru"];
...
const load=L=>{let o={...JSON.parse(fs.readFileSync(`${C}/${L}.json`))};for(const n of NS)Object.assign(o,JSON.parse(fs.readFileSync(`${C}/${L}/${n}.json`)));return Object.fromEntries(flat(o));};
...
> **It currently reports `13 FAILED`, and that is the expected output.** Read the
> three lists separately:
>
> - **`phb` (placeholder mismatch) must be empty.** It is. This is the only part
>   of the check nothing else covers, and the only part that is a hard gate.
> - **`extra`** is plural-suffix noise, not a defect: `ru` carries `_few`/`_many`
>   and `fr`/`it`/`es-MX`/`pt-BR` carry `_many`, none of which `en` has, because
>   their CLDR rules emit categories English does not.
> - **`miss`** is the 130 English-only keys catalogued as **A1** — real
>   untranslated debt, tracked against a baseline by `npm run lint:i18n` so it
>   cannot silently grow. Compare against that baseline, not against zero.
>
> So: treat a non-empty `phb`, or an `A1` count above baseline, as the failure.
