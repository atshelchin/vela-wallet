...
3. 解析 attestation 提取 P-256 公钥 → `computeAddress()` CREATE2 推导(`vela-core` facade → `rust/crates/vela-core/src/safe.rs`；2026-08 前在 `src/services/safe-address.ts`),Safe 此时**未部署**(counterfactual),首笔交易时通过 initCode 部署
