...
> **⚠ 2026-08-13 注**:polyfills 平台对、`.web.ts` Metro 解析等题目描述的机制已随 PR #168
> 的平台对合并而退役(现只有单一实现)。这些题保留作历史理解题;答题时先说明现状再答当年机制。
...
考察范围:Web 构建部署链与回滚、CI 门禁与故意排除项、密钥全景与爆炸半径、跨仓库耦合(vela-relay / getvela.app Worker)、生产排障序列——"怎么上线、坏了怎么救、钥匙在谁手里"。
...
**追问**:
1. vela-relay 明天把文案改成 'gas balance too low, top up 0x…',parse 会命中吗?逐个正则走一遍。
...
**题干**:场景:昨天 vela-relay 仓库合入了一个"改进错误提示措辞"的小 PR。今天 14:00 起,一键 bug report 涌入十几条"发不出交易,弹了一个看不懂的英文报错"。14:40 你确认:gas 账户不足的用户看到的是原始报错,充值 modal 不再弹出。请主持一次复盘:(a) 重建故障链——从 bundler 改文案到用户看到原始报错,中间每一环发生了什么;(b) 本应拦住它的防线有哪几道,为什么全部漏过;(c) 给出立即修复、短期加固、长期根治三层行动项;(d) 这次事故暴露的最深层问题是什么——代码问题还是流程问题?
