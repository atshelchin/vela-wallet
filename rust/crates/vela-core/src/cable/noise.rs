...
fn aes_gcm_seal(key: &[u8], nonce: &[u8; 12], aad: &[u8], pt: &[u8]) -> Result<Vec<u8>, NoiseError> {
...
fn aes_gcm_open(key: &[u8], nonce: &[u8; 12], aad: &[u8], ct: &[u8]) -> Result<Vec<u8>, NoiseError> {
    let cipher = Aes256Gcm::new_from_slice(key).map_err(|_| NoiseError::Cipher)?;
    let nonce = Nonce::from(*nonce);
    cipher
...
impl NoiseInitiator {
    ...
    pub fn write_message1(&mut self, psk: &[u8], ephemeral: KeyPair) -> Result<Vec<u8>, NoiseError> {
...
mod tests {
    ...
    fn a_short_reply_is_rejected() {
        ...
        let _ = initiator
            ...
            .unwrap();
        assert_eq!(initiator.read_message2(&[0u8; 80]), Err(NoiseError::BadLength));
    }
