...
- [X] T032 [US2] Validation & CI: `npx tsc --noEmit`, `npx jest --ci`, `npm run build:web` all green; extend CI `rust` job with `cargo build --target wasm32-unknown-unknown -p vela-core-wasm` + `node rust/scripts/build-web.mjs --check` (verifies committed pkg-web is current). **SC-003 web checklist run 2026-07-28** with the diff harness armed from boot (`localStorage['vela.coreDiff']='1'`, logged at startup) in the parallel space: cold boot (identity derivation + address display), wallet creation (passkey registration → `extract_attestation_public_key`, `validate_client_data`), restore/sign-in, send flow through the confirm sheet (ABI encoding, checksum, quantity), and all 27 clear-signing scenarios (`decode_calldata`, `match_selector`). ~1,900 core calls across 15 core functions → **0 mismatches**. NOT reachable without a local bundler + WalletPair relay, so still unverified in-app (covered by the corpus on all four surfaces, but not by the app's wiring): sign-time `webauthn_signing_hash` / `der_signature_to_raw_low_s` / `hash_typed_data` / `encode_type`, the `recover_public_key_from_assertions` escape hatch, and `sha256` / base64url / `create2_address` / splitter helpers.
