...
'select-token —tap X→ close    —tap token row→ details',
'details —tap ArrowLeft→ select-token    —tap contact icon→ ContactPicker sheet    —tap row QR→ QRScanner',
'details —split/sweep toggles→ multi-mode    —tap import→ BatchImportSheet',
'details —valid input + tap Continue→ confirm',
