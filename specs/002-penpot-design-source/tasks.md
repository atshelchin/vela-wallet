...
- [x] T043 W3b (`551f449`, `771b337`): 93-audit-graph corrected to read `vela.edge` plugin data AND rewritten for cost (its two-pass global findShapeById never finished); root-seeded from the declared flow entries; overlay triggers authored from inventory/07; the signing family declared as one scenario set. **SC-007 PASS** — 87/89 reachable, 2 recorded exclusions, 0 unreachable.
- [x] T044 W4 (`da6b097`, `a4bffaa`, `1c40638`): cover + identity + feature recipe + changelog; principles 07/08 added as specimens (8 of 10 shown, 9 and 10 pointed at their real homes because neither has a still form); reuse index on 09.
- [~] T045 W5 (`6fa480a`): five audits green — 96 floor PASS (89 boards, 0 flat, 0 position mismatch, 3 recorded recapture-debt) · 93 graph PASS · 97 library PASS · 26 token parity PASS (147/147) · 90 idempotency PASS (242 boards, 13,339 shapes, zero diff on a full ordered re-run = SC-005). REMAINING: the US5 agent gate and the SC-008 human gate.
