...
- [X] T001 Commit `design/contacts/` mocks to git on branch
      `018-contacts-ui` (currently untracked)

## Phase 2: Foundational (blocking all platforms)

- [X] T002 [US4] Apply contracts/i18n-keys.md: +21 keys and 2 value
...
- [X] T003 [US4] `npm run gen:i18n` (bump PATHS/leaf pins with ledger
...
- [X] T010 [P] [US1] Icon corpus: add the 11 contracts/icons.json glyphs
      to `app-web/vela-wallet/src/lib/wallet/icons.ts`
- [X] T011 [US1] `src/lib/contacts/model.ts` (display models,
...
- [X] T012 [US4] `resolveContactsMessages(locale)` in
...
- [X] T013 [P] [US1] New components in `src/lib/contacts/ui/`:
...
- [X] T020 [P] [US2] `src/icons.rs`: add the 11 glyph bodies;
...
- [X] T021 [US2] `src/contacts/` module: `mod.rs` (ContactsStrings
...
- [X] T022 [US2] `src/wallet/page.rs`: `Section { Wallet, Contacts }`
...
- [X] T023 [US2] Gallery: `GalleryTab` += DC1…DC6 + ContactsComponents
      board; `VELA_PAGE=contacts` arm in `main.rs`; gates: `cargo check
      && cargo clippy --all-targets && cargo test`

## Phase 5: US1 (+US3) on Android

- [X] T030 [P] [US1] `VelaIcons.kt`: add the 11 glyphs (PathParser,
      24×24, matching contracts/icons.json)
- [X] T031 [US4] `core/i18n/I18nKeys.kt`: `object Contacts` with the
      full key map
- [X] T032 [US1] `feature/contacts/`: ContactsModels.kt (+
...
- [X] T040 [P] [US1] `DesignSystem/LucideIcons.swift`: add the 11
      glyphs + size entries; keep `audit-literals.mjs` clean
- [X] T041 [US1] `Features/Contacts/`: ContactsModels.swift
...
- [X] T042 [US1] Screens: ContactsScreen / ContactDetailScreen /
...
- [X] T043 [US3] Behaviours: index-rail DragGesture + selection haptics
      + bubble overlay; swipe reveal via horizontal drag anchors;
      `accessibilityReduceMotion` degrade
- [X] T044 [US1] `ContactsGalleryScreen` + `VELA_PAGE=contacts|
...
- [~] T050 [P] Cross-platform identicon spot check: SC-006 reuse verified
      by import inspection on all four platforms (no duplicated 015
      components — see results.md deviation 6). Pairwise identicon
      eyeballing is blocked on desktop/Android hardware (results.md
      "Verification gaps")
- [X] T051 [P] zh ↔ en walkthrough on every gallery (US4 SC — no key
      echoes, no clipped strings at 1.35×)
- [X] T052 `specs/018-contacts-ui/results.md`: gates table + recorded
      deviations (8-roster/妈妈, invented-address identicons, desktop
      <1120 N/A, plus anything discovered); update checklists
- [X] T053 Full gate sweep per platform; commits per phase (core, web,
