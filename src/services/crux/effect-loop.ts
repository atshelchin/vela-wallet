 ...
 *
 * Adapted from the reference implementation in crux-demo, trimmed to what this
 * app needs (no devtools timeline, no agent-bridge origins). Every core in this
 * repo shares it, which is why the failure and cancellation semantics live here
...
export function createEffectLoop<View, Event, Effect extends EffectWithId, Result>(
  ...
  function commit(view: View) {
    // A view produced before disposal can still arrive after it (an effect that
    // resolves while the screen unmounts). Dropping it keeps React from being
    // asked to render a torn-down tree.
    if (disposed) return;
    options.onView(view);
  }
