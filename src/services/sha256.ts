 ...
 *
 * Synchronous and dependency-free so it runs identically on Hermes, web, and
 * Node — `crypto.subtle.digest` is async and not available on Hermes, and the
 * only hashing dependency in the app (js-sha3) is Keccak, not SHA-2.
 * Style-wise this mirrors the hand-rolled Keccak in eth-crypto.ts.
 */
...
/** Compute the SHA-256 digest of `data`. Returns a fresh 32-byte array. */
export function sha256(data: Uint8Array): Uint8Array {
  ...
  // Message lengths here are far below 2^32 bits — high word stays 0.
  ...
  [h0, h1, h2, h3, h4, h5, h6, h7].forEach((word, i) => outView.setUint32(i * 4, word));
  return out;
}
