...
import { getLogsRangeCap, poolRpcCall } from '@/services/rpc-pool';
...
import type { TrustCustomToken } from './generated/TrustCustomToken';
import type { TrustMetaEntry } from './generated/TrustMetaEntry';
import type { TrustRawLog } from './generated/TrustRawLog';
...
function toWireLog(raw: unknown): TrustRawLog {
  ...
  const topics = Array.isArray(record.topics)
    ? (record.topics as unknown[]).map(asString)
    : [];
  return {
...
export async function executeTokenTrustOperation(
  effect: TrustEffect,
): Promise<TrustShellResult> {
  ...
  switch (operation.type) {
    ...
    case 'rpc_get_block_by_number': {
      ...
      const timestamp = !res?.error ? (res?.result as { timestamp?: unknown })?.timestamp : undefined;
      return {
        ...
        timestamp_sec:
          typeof timestamp === 'string' ? hexToNumber(timestamp) : null,
        // ③ the clock, carried.
        now_ms: Date.now(),
      };
    }
...
export function tokenTrustOperationFailure(
  effect: TrustEffect,
  _error: unknown,
): TrustShellResult {
  const operation = effect.operation;
