...
export function ensureTxTracker() {
  if (!session) {
    session = createTxTrackerSession({
      ...
      ports: {
        feedReconciled: notifyFeedReconciled,
        receiptLogsConfirmed: notifyReceiptLogsConfirmed,
      },
    });

    // The `sign_request` seam. A dApp tx that reached the bundler is handed
    // over the moment `view.tracker_handoff` appears — before the receipt wait,
    // which is now this machine's. The sign sheet consumes no outcome (it has
    // already answered the dApp and flipped its own record), so no watcher.
    setSignTrackerSink((handoff) => {
      dispatchTxTracker({
        type: 'submitted',
...
export function dispatchTxTracker(event: TrackEvent): void {
  ensureTxTracker().dispatch(event);
}
