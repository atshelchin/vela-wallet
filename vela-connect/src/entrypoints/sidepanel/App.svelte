<script lang="ts">
  ...
  function startScan() {
    chrome.runtime.sendMessage({ type: 'VELA_POPUP_ACTION', action: 'startScan' });
  }

  function disconnect() {
    // Close pairing tab to disconnect BLE
  ...
  function methodLabel(method: string): string {
    return ({ eth_sendTransaction: 'Transaction', personal_sign: 'Sign Message', eth_signTypedData_v4: 'Sign Typed Data' } as Record<string, string>)[method] || method;
  }
...
<div class="header">
  ...
  <div class="badge" class:off={connectionState === 'disconnected'} class:searching={connectionState === 'searching'} class:on={connectionState === 'connected'}>
    ...
    {:else if connectionState === 'searching'}Connecting…
    {:else}Connected{/if}
  </div>
</div>
...
<div class="body">
  {#if connectionState === 'connected' && activeRequest}
    <div class="dapp-origin">
      {#if activeRequest.favicon}<img class="dapp-favicon" src={activeRequest.favicon} alt="" />{/if}
    ...
    <div class="tx-card">
      ...
      <div class="tx-row"><span class="tx-label">From</span><span class="tx-value">{walletInfo ? shortAddr(walletInfo.address) : '...'}</span></div>
    </div>
    <div style="text-align:center;padding:20px 0;">
      <div class="spinner"></div>
      <div style="font-size:15px;font-weight:500;color:var(--text-1);">Confirm on phone</div>
      <div style="font-size:13px;color:var(--text-3);margin-top:4px;">Check Vela Wallet</div>
    </div>
  ...
  {:else if connectionState === 'connected'}
    <div class="device-card">
      ...
      <div>
        ...
        <div class="device-status"><div class="badge-dot" style="width:5px;height:5px;background:var(--green);border-radius:50%;"></div>Bluetooth connected</div>
      </div>
    </div>
    ...
    <div style="margin-top:auto;padding-top:20px;"><button class="btn btn-red" onclick={disconnect}>Disconnect</button></div>

  {:else}
    <div class="center-state">
      <div class="center-icon" style="background:var(--blue-soft);">📱</div>
      <div class="center-title">Connect your phone</div>
      <div class="center-desc">Pair with Vela Wallet on your phone via Bluetooth.</div>
      <button class="btn btn-blue" onclick={startScan}>Pair with phone</button>
    </div>
  {/if}
</div>

<div class="footer"><span>Vela Connect v1.0.0</span></div>
